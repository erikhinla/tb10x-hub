...
const SocialAssetHub = () => {
  ...
  const exportMediaType =
    mediaType === "video" && capturedVideoFrame ? "image" : mediaType;
  const exportImageUrl =
    exportMediaType === "image"
      ? mediaType === "video"
        ? capturedVideoFrame ?? undefined
        : activeImageUrl
      : undefined;
  const exportVideoUrl = exportMediaType === "video" ? activeVideoUrl : undefined;
    ...
    const hydrateFromControlTask = async () => {
      ...
      try {
        ...
        if (preset) {
          ...
          if (preset.supportLine) setSupportLine(preset.supportLine);
          if (preset.seriesLabel) setSeriesLabel(preset.seriesLabel);
          if (preset.mediaType) setMediaType(preset.mediaType);
          if (preset.imageUrlInput) setImageUrlInput(preset.imageUrlInput);
          if (preset.videoUrlInput) setVideoUrlInput(preset.videoUrlInput);
          if (preset.ratioPreset) setRatioPreset(preset.ratioPreset);
        }
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="grid gap-2">
              <button
                ...
                className={`border px-3 py-2 text-[0.58rem] uppercase tracking-[0.16em] transition-colors ${
                  contentMode === "internal_asset"
        ...
        <section className="pt-10">
          ...
          <div className="mt-4 border border-[#c4beaf]/20 bg-[#090b0f] p-4 sm:p-5">
            <div className="mx-auto w-full max-w-4xl">
              <div
                ref={exportStageRef}
                className={`mx-auto w-full max-w-[48rem] overflow-hidden bg-[#07080a] ${selectedRatio.aspectClass}`}
              >
                <AssetPreviewCard
          ...
          {mediaType === "video" ? (
            <p className="mt-2 text-[0.52rem] uppercase tracking-[0.16em] text-[#8b92a0]">
              Video mode exports a still frame as PNG.
            </p>
          ) : null}
        </section>
